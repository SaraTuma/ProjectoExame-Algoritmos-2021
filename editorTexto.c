...
void executar ( int cod )
{
	...
	"Linha corrente não encontrada!","Ao acessar o ficheiro de texto!"};
	if(cod==10) printf (" %s\n",  msg[cod] );
	else if(cod!=0){
		printf ("\nERRO: %s\n",  msg[cod] );
	}
...
int verificarNumeros(TLista *lista,TPilha * pilha,int a, int b,int n,int op)
{
	...
	if(b<=0 ) return SECUND_INVALID;
	if(op==1)
		return imprimirLista(*lista,a,b);
	else{
		
		if(a==b)
			return removerAtomoChave(lista,a);
		return removerAtomosLista(lista,a,b);
	} 
}
...
int afastarCaracteres(char * str, int qtd, int ini,int op){
	...
	while(i<=qtd){
		...
		if(op==1){
			...
			executar(arrastarString(str,ini,size));
			size++;
		}
		if(op==2){
			//remover caracteres
			executar(arrastarStringFim(str,ini,size));
		}	
...
int alterarFrase(TLista lista, char * substring, char * frase)
{
	...
	int pos,cont=0,aux=0,tam=0, encontrei=0;	
	...
	for(int i=0;paux->info.frase[i]!='\0';i++){
			...
			if(cont==sizeS){
                ...
                if(sizeF==0)
                    executar(afastarCaracteres(paux->info.frase,sizeS,i,2));
				else if(sizeF>sizeS) 
					executar(afastarCaracteres(paux->info.frase,sizeF-sizeS,sizeS+i,1));
				else if(sizeF<sizeS)
					executar(afastarCaracteres(paux->info.frase,sizeS-sizeF,sizeF+i,2));
				aux=0;
...
int removerFrase(TLista * lista,TPilha * pilha, char * frase)
{
		...
		if(verificarStrings(paux->info.frase,frase))
			{
				...
				return removerAtomoChave(lista,paux->info.chave);
			}
	}
	...
	for(int i=0;paux->info.frase[i]!='\0';i++){
			...
			if(cont==size){
				executar(afastarCaracteres(paux->info.frase,size,i,2));	
				encontrei=1;
...
* Objectivo : recuperar uma linha que foi removida
* Parametros :
	* lista - a lista de atomos
	* pilha - a pilha de atomos
...
int recuperarLinha(TLista * lista, TPilha * pilha,PAtomo *paux){
	...
	lista->nElem++;
	executar ( actualizarLista(lista));
...
int iniciarEditor(){
	...
	TPilha pilha;
	TInfo x;
	int run = 1, ler = 0, cont = 0,comando =0;
	inicializarPilha(&pilha);
	inicializarLista(&lista);
	inicializarLista(&nova);
	while (run){
		x.frase  = (char*)(malloc(sizeof(char) * TAM));
		...
		if(*x.frase == '$'){
			...
			if(comando==COMANDO_INVALIDO){
				...
				if (verifOperacao(x.frase, "$inserir"))
				{
					...
					if(!vaziaPilha(pilha)){
						executar(recuperar(&lista,&pilha));
					}
			...
			if(tamanhoString(x.frase)>0){
				executar(inserirAtomoFim(&nova, x));
			}
		}
		else{
